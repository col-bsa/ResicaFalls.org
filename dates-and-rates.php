<!DOCTYPE html>
<html lang="en">
	<head>
		<?php include '_inc/html_head.php'; ?>
		<meta name="description" content="Explore the pricing and availability at Resica Falls Scout Reservation." />
		<title>Dates &amp; Rates <?php include '_inc/var/site_name.php'; ?></title>
		<!-- Google Tag Manager -->
		<script>(function(w,d,s,l,i){w[l]=w[l]||[];w[l].push({'gtm.start':new Date().getTime(),event:'gtm.js'});var f=d.getElementsByTagName(s)[0],j=d.createElement(s),dl=l!='dataLayer'?'&l='+l:'';j.async=true;j.src='https://www.googletagmanager.com/gtm.js?id='+i+dl;f.parentNode.insertBefore(j,f);})(window,document,'script','dataLayer','GTM-5R8JMMS');</script>
		<!-- End Google Tag Manager -->
	</head>
	<body>
		<?php include '_inc/header.php'; ?>
		<?php include '_inc/nav.php'; ?>

		<div class="container content">
			<div class="row">
				<div class="col">
					<h1>2021 Dates &amp; Rates</h1>
				</div>
			</div>
			<div class="row">
				<div class="col-12 col-md-6">
					<table class="table text-center table-sessions">
						<thead class="thead-inverse">
							<tr>
								<th class="text-center">Session</th>
								<th class="text-center">Dates</th>
								<th class="text-center">Registration</th>
							</tr>
						</thead>
						<tbody>
							<tr>
								<td>Week 1</td>
								<td>June 27 - July 3</td>
								<td><a class="btn btn-success" href="https://colbsa.doubleknot.com/event/scouts-bsaventuring-summer-camp-at-resica-falls-week-1/2678053">Register</a></td>
							</tr>
							<tr>
								<td>Week 2</td>
								<td>July 4 - July 10</td>
								<td><a class="btn btn-success" href="https://colbsa.doubleknot.com/event/scouts-bsaventuring-summer-camp-at-resica-falls-week-2/2693058">Register</a></td>
							</tr>
							<tr>
								<td>Week 3</td>
								<td>July 11 - July 17</td>
								<td><a class="btn btn-success" href="https://colbsa.doubleknot.com/event/scouts-bsaventuring-summer-camp-at-resica-falls-week-3/2693059">Register</a></td>
							</tr>
							<tr>
								<td>Week 4</td>
								<td>July 18 - July 24</td>
								<td><a class="btn btn-success" href="https://colbsa.doubleknot.com/event/scouts-bsaventuring-summer-camp-at-resica-falls-week-4/2693060">Register</a></td>
							</tr>
							<tr>
								<td>Week 5</td>
								<td>July 25 - July 31</td>
								<td><a class="btn btn-success" href="https://colbsa.doubleknot.com/event/scouts-bsaventuring-summer-camp-at-resica-falls-week-5/2693061">Register</a></td>
							</tr>
							<tr>
								<td>Week 6</td>
								<td>August 1 - August 7</td>
								<td><a class="btn btn-success" href="https://colbsa.doubleknot.com/event/scouts-bsaventuring-summer-camp-at-resica-falls-week-6/2693062">Register</a></td>
							</tr>
							<tr>
								<td>Week 7</td>
								<td>August 8 - August 14</td>
								<td><a class="btn btn-success" href="https://colbsa.doubleknot.com/event/scouts-bsaventuring-summer-camp-at-resica-falls-week-7/2693063">Register</a></td>
							</tr>
						</tbody>
					</table>
				</div>
				<div class="col-xl-6">
					<table class="table text-center">
						<thead class="thead-inverse">
							<tr>
								<th class="text-center">Pricing</th>
							</tr>
						</thead>
						<tbody>
							<tr class="table-highlight">
								<td>Early Bird: $440 per Youth / $200 per Adult</td>
							</tr>
							<tr>
								<td>Regular: $465 per Youth / $200 per Adult</td>
							</tr>
							<tr>
								<td><em>Early bird offer ends December 15</em></td>
							</tr>
							<tr>
								<td><strong>Second Week: $250 per Youth</strong></td>
							</tr>
							<tr>
								<td>
									<strong>Every unit receives 2 free adults!</strong><br>
									You will also earn 1 free adult for every 10 scouts after your first 20 scouts!
								</td>
							</tr>
						</tbody>
					</table>
					<a class="btn btn-primary btn-block" href="https://colbsa.doubleknot.com/Rosters/logon.aspx?orgkey=541">View Your Current Registration</a>
				</div>
			</div>
			<div class="row">
				<div class="col-xl-6">
					<h2>Campsites</h2>
					<div class="table-responsive">
						<table class="table table-sm text-center">
							<caption>All campsites have a fire ring, picnic tables, tarp, flagpole, and latrine with water in-site.</caption>
							<thead class="thead-inverse">
								<tr>
									<th class="text-left">Name</th>
									<th class="text-center">Capacity</th>
									<th class="text-center">Pavilion</th>
								</tr>
							</thead>
							<tbody>
								<tr>
									<th scope="row" class="text-nowrap">Bear Paw</th>
									<td>40</td>
									<td><i class="fa fa-check"></td>
								</tr>
								<tr>
									<th scope="row" class="text-nowrap">Cayuga</th>
									<td>84</td>
									<td></td>
								</tr>
								<tr>
									<th scope="row" class="text-nowrap">Cherokee</th>
									<td>56</td>
									<td></td>
								</tr>
								<tr>
									<th scope="row" class="text-nowrap">Cheyenne</th>
									<td>56</td>
									<td></td>
								</tr>
								<tr>
									<th scope="row" class="text-nowrap">Chinook</th>
									<td>52</td>
									<td></td>
								</tr>
								<tr>
									<th scope="row" class="text-nowrap">Chippewa</th>
									<td>46</td>
									<td></td>
								</tr>
								<tr>
									<th scope="row" class="text-nowrap">Comanche</th>
									<td>72</td>
									<td></td>
								</tr>
								<tr>
									<th scope="row" class="text-nowrap">Deer Run</th>
									<td>40</td>
									<td><i class="fa fa-check"></td>
								</tr>
								<tr>
									<th scope="row" class="text-nowrap">Little Mohawk</th>
									<td>20</td>
									<td></td>
								</tr>
								<tr>
									<th scope="row" class="text-nowrap">Mohawk</th>
									<td>30</td>
									<td></td>
								</tr>
								<tr>
									<th scope="row" class="text-nowrap">Navajo</th>
									<td>56</td>
									<td></td>
								</tr>
								<tr>
									<th scope="row" class="text-nowrap">North Wind</th>
									<td>50</td>
									<td></td>
								</tr>
								<tr>
									<th scope="row" class="text-nowrap">Pueblo</th>
									<td>36</td>
									<td></td>
								</tr>
								<tr>
									<th scope="row" class="text-nowrap">Running Fox</th>
									<td>42</td>
									<td></td>
								</tr>
								<tr>
									<th scope="row" class="text-nowrap">Seneca</th>
									<td>36</td>
									<td></td>
								</tr>
								<tr>
									<th scope="row" class="text-nowrap">Shawnee</th>
									<td>42</td>
									<td></td>
								</tr>
								<tr>
									<th scope="row" class="text-nowrap">Shenandoah</th>
									<td>38</td>
									<td></td>
								</tr>
								<tr>
									<th scope="row" class="text-nowrap">Shoshone</th>
									<td>50</td>
									<td></td>
								</tr>
								<tr>
									<th scope="row" class="text-nowrap">Troop One</th>
									<td>16</td>
									<td></td>
								</tr>
								<tr>
									<th scope="row" class="text-nowrap">Twin Hill</th>
									<td>40</td>
									<td></td>
								</tr>
								<tr>
									<th scope="row" class="text-nowrap">Two Feathers</th>
									<td>52</td>
									<td></td>
								</tr>
							</tbody>
						</table>
					</div>
				</div>
			</div>
		</div>
		<?php include '_inc/footer.php'; ?>
		<?php include '_inc/html_foot.php'; ?>
	</body>
</html>
